...
TOOL_SPEC = ToolSpec(
    name="find_history",
    description=(
        "Ищет по СВОЕЙ прошлой переписке этого пользователя с ботом (его прошлые вопросы и ответы на них) — "
        "личная память, а не база знаний. Использовать для запросов вида «найди в истории/базе переписки X», "
        "«что я спрашивал про X», «напомни, что ты говорил про X». НЕ использовать для обычных вопросов по теме "
        "проекта — на них нужно отвечать напрямую, а не искать в истории."
    ),
    parameters=[ToolParameter(name="query", type="string", description="Текст для поиска")],
    handler=run,
    needs_user_id=True,
)
